...
(function ($) {
    $.extend({
        ...
        form: {
            ...
            select2DefautSelect:function(id,defVal){
                $(id).val(defVal).select2({
                    width: "150px"
                });
            },
            // 获取选中复选框项
        ...
        file: {
            /**
             * 附件上传
             * @param fileEl   附件文件选择器
             * @param formEl   表单附件字段选择器
             */
            fileUpload: function (fileEl, formEl) {
                $(fileEl).fileinput({
                    language: 'zh',
                    uploadUrl: "/sysFileInfo/batchAddSysFileInfo",
                    layoutTemplates: {},
                    previewTemplates: '',
                    ...
                    if (d.response.code === 500) {
                        alert(d.response.msg);
                        return;
                    }
                    let fileDataArray = d.response.data;
                    $('#' + previewId).prop('fileid', fileDataArray[0].id);
                    let splitUUid = $.common.getSplitFileUUID();
                    $(formEl).val(JSON.stringify(fileDataArray[0]) + splitUUid + $(formEl).val());
                }).on('filesuccessremove', function (event, previewId, extra) {
                    let splitUUid = $.common.getSplitFileUUID();
                    var delId = $('#' + previewId).prop('fileid');
                    var aboutFileStrArray = $(formEl).val().split(splitUUid);
                    var finalVal = "";
                    for (const aboutFileStrItem of aboutFileStrArray) {
                        if (aboutFileStrItem !== "") {
                            var aboutFileItem = JSON.parse(aboutFileStrItem);
                            if (aboutFileItem.id !== delId) {
                                finalVal += (aboutFileStrItem + splitUUid);
                            }
                        }
                    }
                    $(formEl).val(finalVal);
             ...
             * @param fileEl   附件文件选择器
             * @param formEl   表单附件字段选择器
             * @param flieList  初始化文件列表
             */
            editFileUpload: function (fileEl, formEl, flieList) {
                var fileNames = [];
                var fileCaptions = [];
                for (var fileItem of flieList) {
                    fileNames.push(fileItem.visitUrl);
                    var obj = {
                        caption: fileItem.name,
                        size: fileItem.fileSize,
                        key: fileItem.id,
                        url: '/sysFileInfo/delSysFileInfo',
                        type: 'other'
                    };
                    fileCaptions.push(obj);
                }
                $(fileEl).fileinput({
                    language: 'zh',
                    uploadUrl: "/sysFileInfo/batchAddSysFileInfo",
                    initialPreview: fileNames,
                    previewFileType: 'any',
                    overwriteInitial: false,
                    initialPreviewAsData: true,
                    initialPreviewConfig: fileCaptions,
                    layoutTemplates: {},
                    previewTemplates: '',
                    otherActionButtons: '<button type="button" {dataKey} class="kv-file-down btn btn-sm btn-kv btn-default btn-outline-secondary" title="下载"><i class="glyphicon glyphicon-download"></i></button>',
                    showUpload: false,
                    showRemove: false,
                    showCaption: true,
                    showClose: false,
                    ...
                    var aboutFileStrArray = $(formEl).val().split(splitUUid);
                    var finalVal = "";
                    for (let i = aboutFileStrArray.length - 1; i >= 0; i--) {
                        const aboutFileStrItem = aboutFileStrArray[i];
                        if (i == aboutFileStrArray.length - 1) {
                            var oldFileArray = aboutFileStrItem.split(",");
                            for (let oldFile of oldFileArray) {
                                if (oldFile !== delId) {
                                    finalVal += (oldFile + splitUUid);
                                }
                            }
                        } else {
                            if (aboutFileStrItem !== "") {
            ...
            downloadHandler: function (fileobj) {
                ...
                objs.on("click", function () {
                    //点击下载
                    debugger;
                    window.location.href = "/common/newResourceDownload/" + $(this).data("key");
                });
            },
            getFileByIds: function (ids, callback) {
                var config = {
                    url: "/sysFileInfo/getSysFileInfoByIds",
                    type: "post",
                    dataType: "json",
                    data: {"ids": ids},
                    success: function (result) {
        ...
        operate: {
            ...
            // 保存信息 刷新表格(带附件)
            saveFile: function (url, formdata, callback) {
                var config = {
                    url: url,
                    type: "post",
                    cache: false,
        ...
        // 通用方法封装处理
        common: {
            getSplitFileUUID: function () {
                return "2fd872b8933a4ea7bee38e1c7bd96feb";
            },
            // 判断字符串是否为空
